...
public class NicolasRoseDrawing extends RoseDrawing {
	...
	private void drawOrientation(Canvas pCanvas, String orientationLabel, float x, float y,float angle) {
		...
		final float AREA_DIAMETER = DIRECTION_CIRCLE_AREA_RADIUS*2.0f;
		
		Paint paint = new Paint();
		paint.setAntiAlias(true);
		paint.setMaskFilter(new BlurMaskFilter(2, BlurMaskFilter.Blur.SOLID));
		Bitmap bm = Bitmap.createBitmap((int)AREA_DIAMETER+2, (int)AREA_DIAMETER+2, Bitmap.Config.ARGB_8888);
		Canvas c = new Canvas(bm);
	...
	public DrawingComponent[] getComponents() {
		if (components == null) {
			...
			// direction labels
			final float TEXT_AREA_FACTOR = 2.5f;
			Figure d0Label = FRect.fromMiddle(0, -DIRECTION_RING_RADIUS, DIRECTION_CIRCLE_AREA_RADIUS * TEXT_AREA_FACTOR);
			Figure d90Label = FRect.fromMiddle(DIRECTION_RING_RADIUS, 0, DIRECTION_CIRCLE_AREA_RADIUS * TEXT_AREA_FACTOR);
			Figure d180Label = FRect.fromMiddle(0, DIRECTION_RING_RADIUS, DIRECTION_CIRCLE_AREA_RADIUS * TEXT_AREA_FACTOR);
			Figure d270Label = FRect.fromMiddle(-DIRECTION_RING_RADIUS, 0, DIRECTION_CIRCLE_AREA_RADIUS * TEXT_AREA_FACTOR);
			...
			DrawingComponent compDirectionCircleAreas = new DrawingComponent(0, DIRECTION_CIRCLE_AREA_NAME, degreeLabels,directionCircleAreaColor,resources.getString(R.string.color_choose_DIRECTIONS_LABEL_NAME));
			
			// Build array of the components
			components = new DrawingComponent[] { compOuterRing, compBottom, compDirectionCircleAreas };
