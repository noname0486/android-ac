...
import de.rothbayern.android.ac.pref.ColorPickerPreferenceView;
...
// TODO some code to remove	
	...
	private static final int SLIDER_THICKNESS = 60;

	private static final int VERT_SLIDER_POS_X = PALETTE_DIM+SLIDER_THICKNESS/2;
	 ...
	 * @throws Exception
	 */
	public ColorPickerView(Context c, OnColorChangedListener l, int width, int height, int color) {
		super(c);
		...
		if (UberColorPickerDialog.isGray(color))		//Ugh, I think there's a bug in android's Color routines.  Read the longer description at isGray().
			mHSV[1] = 0;
...
*/		
	...
	private void drawSwatches(Canvas canvas) {
		...
		String sOldColor = getContext().getResources().getString(R.string.color_picker_old_color);
		drawTextMiddle(canvas, sOldColor, mOldSwatchRect, mText);
		mText.setColor(Color.WHITE);
		...
		if (mHSV[2] > .5)
			mText.setColor(Color.BLACK);
		String sNewColor = getContext().getResources().getString(R.string.color_picker_new_color);
		drawTextMiddle(canvas, sNewColor, mNewSwatchRect, mText);
		mText.setColor(Color.WHITE);
	...
	private static void drawTextMiddle(Canvas canvas, String text, Rect r, Paint p){
		float length = p.measureText(text);
		canvas.drawText(text, r.left + (r.width()-length)/2, r.top + (r.height()+TEXT_SIZE)/2, p);
	}
...
*/
	...
	private void drawHSV1Palette(Canvas canvas) {
		...
		//Draw the 1D slider
		canvas.translate(VERT_SLIDER_POS_X, 0);
...
*/	
	...
	private void updateHSVfromRGB() {
		...
		if (UberColorPickerDialog.isGray(mRGB))
			mHSV[1] = 0;
	}
...
*/
			...
			case MotionEvent.ACTION_UP:
				...
				if (mTracking == TRACK_SWATCH_OLD && inSwatchOld) {
					...
					if (UberColorPickerDialog.isGray(mOriginalColor))
						mHSV[1] = 0;
